...
sources.forEach(source => {
  ...
  fs.writeFileSync(
    path.join(bundlesDir, `hydrate.${name}.${type}.js`),
    hydrated,
    'utf8'
  );
  const clientOnly = generator(name, false);
  fs.writeFileSync(
    path.join(bundlesDir, `client-only.${name}.${type}.js`),
    clientOnly,
    'utf8'
  );
});
...
function generateStyletronBundle(name) {
  return `const styletronApp = require('../app/${name}.styletron');
...
function generateGlamorBundle(name, hydrate) {
  const hydrateSrc = hydrate ? 'glamor.rehydrate(window._glam);' : '';
...
ssStyles.parentNode.removeChild(ssStyles);`
    : '';
  return `const create = require('jss').create;
