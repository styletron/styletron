    ...
    if (match) {
      ...
      for (let key in match) {
        if (hyphenated[key]) {
          ...
          console.error(
            `Warning: both "${original}" and "${problem}" are used in style object: ${JSON.stringify(stylesObject)}. Mixing of shorthand and longform properties is unsupported and may result in unexpected style behavior. You may want to use the https://lostyle.js.org/global.html#${prop} helper function to convert from shorthand to longform properties.`
          );
